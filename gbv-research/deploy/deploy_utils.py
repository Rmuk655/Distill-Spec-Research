...
def install_deps(gbv_dir: str = GBV_DIR) -> None:
    ...
    # Force-upgrade bitsandbytes FIRST with -U so pip doesn't skip it because the
    # pre-installed base-image version (e.g. 0.43/0.44) satisfies the old >=0.43
    # constraint.  transformers hard-requires >=0.46.1 for 4-bit NF4; without -U,
    # pip treats any installed >=0.43 as "already satisfied" and never upgrades.
    # The prebuilt >=0.46.1 wheels ship working CUDA kernels for torch 2.10, so NF4
