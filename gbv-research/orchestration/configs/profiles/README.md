...
| `train_one_loss` | `profiles/train_one_loss` | **Phase-1 workhorse** — tiered: train ONE flat loss + in-training val_loss + light BE sanity | Yes (full steps) | Light: matched verifier per loss (e.g. `bv` for `kl`), K=3, T=1.0, n=100, GSM8K only |
| `tree_variant_week` | `profiles/tree_variant_week` | **Phase-2 tree survivor** — tiered: train ONE tree variant + light BE sanity | Yes (full steps) | Light: matched verifier per loss (e.g. `gbv` for `gbv_tree`), K=3, T=1.0, n=100 |
...
train  →  val_loss (always-on, inside trainer.py every health.val_every steps)
       →  LIGHT BE sanity (1 matched verifier per loss, n=100, K=3, T=1.0)
```
...
| Phase 3 — light BE sanity | n=5 (smoke) | matched verifier per loss (e.g. `bv` for `kl`), n=100 | matched verifier per loss (e.g. `gbv` for `gbv_tree`), n=100 | — |
| Phase 1 — baseline group 0 | yes (smoke) | dropped by `light_eval` | dropped by `light_eval` | yes |
| Phase 4 — multi-dataset | — | dropped by `light_eval` | dropped by `light_eval` | yes |

---

## `experiment.light_eval_verifier` (optional override)

Both `train_one_loss` and `tree_variant_week` support an optional
`experiment.light_eval_verifier` key that overrides which verifier is used for the
light sanity eval step:

```yaml
experiment:
  light_eval: true
  light_eval_verifier: bv   # force all active losses to use "bv" for light sanity
```

**Default (when `light_eval_verifier` is absent):** each loss automatically uses its
*naturally matched* verifier from `_LOSS_LIGHT_VERIFIER` in `experiment.py`:

| Loss | Default light verifier |
|------|------------------------|
| `kl`, `rev_kl`, `jsd`, `l1`, `ebe`, `ebe_single`, `online*` | `bv` |
| `kl_tree`, `rev_kl_tree`, `jsd_tree`, `ebe_tree` | `bv` |
| `bv_tree` | `bv` |
| `gbv_tree` | `gbv` |
...
When `light_eval_verifier` is set it applies to **all** active losses.
When absent, each loss gets its own matched verifier — the cheapest honest sanity
for that specific surrogate.
