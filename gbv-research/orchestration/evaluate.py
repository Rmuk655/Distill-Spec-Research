...
def run_be(student_path: str, teacher_path: str,
           ...
           draft_temp: float = 1.0, _device: str = "cuda") -> dict | None:
        ...
        if not m:
            ...
            if oom and _device == "cuda":
                ...
                return run_be(student_path, teacher_path, mode, K, L,
                              data_path, max_new_tokens, draft_temp=draft_temp,
                              _device="cpu")
...
def run_be_batch(student_path: str, teacher_path: str, data_path: str,
                 ...
                 L: int, max_new_tokens: int, draft_temp: float = 1.0,
                 _device: str = "cuda",
    ...
    --Ks, --teacher_temps to the verifier script (_VERIFIER_SCRIPT, Phase 2: runner.py)
    ...
    cmd = [
        ...
        "--teacher_temps", ",".join(str(t) for t in temps),
        "--L",        str(L),
        "--draft_temp", str(draft_temp),
        "--max_new_tokens", str(max_new_tokens),
        "--data",     data_path,
...
def _wandb_log_eval_result(row: dict) -> None:
    ...
    try:
        ...
        if row.get("block_eff") is not None:
            _wmod.summary[f"BE/{mode}"] = round(float(row["block_eff"]), 4)
...
def run_cell(student_path: str, teacher_path: str, student_label: str,
    ...
    if mode == "alpha":
        ...
        res = run_be(student_path, teacher_path, mode, K, L, data_path, max_tokens,
                     draft_temp=getattr(args, "draft_temp", 1.0))
        if res is None:
            return {}
...
def main():
    ...
    p.add_argument("--teacher_temps", "--temperature", dest="teacher_temps",
                   default=",".join(map(str, DEFAULT_TEMPS)),
                   help="Comma-separated teacher verification temps at eval (runner p_temp). "
                        "NOT training teacher_temperature. Legacy alias: --temperature.")
    ...
    p.add_argument("--draft_temp", "--q_temp", dest="draft_temp", type=float, default=1.0,
                   help="Draft proposal temperature at eval (runner q_temp). "
                        "Independent of --teacher_temps and training teacher_temperature. "
                        "YAML: evaluation.draft_temp. Legacy alias: --q_temp.")
