...
TRAIN_DATASETS = {
    # name         → (hf_id,                        split,   prompt_field,  config, answer_field, level_filter)
    # level_filter: set of "Level N" strings to keep, or None for all levels
    "gsm8k_train": None,   # handled separately by fetch_gsm8k_train
    "math_train":  ("hendrycks/competition_math",  "train",  "problem",     None,  "solution",   None),
    "math_hard":   ("hendrycks/competition_math",  "train",  "problem",     None,  "solution",   {"Level 4", "Level 5"}),
    "math_val":    ("hendrycks/competition_math",  "test",   "problem",     None,  "solution",   None),
}


def fetch_math_train(name: str, level_filter: set | None = None,
                     n: int = 10_000, force: bool = False) -> str:
    """Fetch MATH train split from hendrycks/competition_math, optionally filtered by level."""
    path = os.path.join(DATA_DIR, f"{name}.jsonl")
    if os.path.isfile(path) and not force:
        print(f"  {name}.jsonl already present — skipping ({path})")
        return path

    label = f"levels {sorted(level_filter)}" if level_filter else "all levels"
    print(f"  fetching {name} (MATH train, {label}) ...")
    try:
        from datasets import load_dataset
...
def fetch_all(n_eval: int = 100, train: bool = False, datasets: list = None,
    ...
    for name, spec in TRAIN_DATASETS.items():
        if name not in requested or spec is None:
            continue
        _, _, _, _, _, level_filter = spec
        fetch_math_train(name, level_filter=level_filter, force=force)
...
def get_path(name: str) -> str:
    ...
    if name in ("gsm8k_train", "gsm8k_val", "gsm8k_eval"):
        return os.path.join(DATA_DIR, f"{name}.jsonl")
    if name in EVAL_DATASETS or name in TRAIN_DATASETS:
        return os.path.join(DATA_DIR, f"{name}.jsonl")
    raise KeyError(f"Unknown dataset '{name}'. Known: gsm8k_train, gsm8k_val, gsm8k_eval, "
                   f"{sorted(EVAL_DATASETS)}, {sorted(TRAIN_DATASETS)}")
