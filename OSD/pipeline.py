...
_LOCK_FILE    = os.path.join(HERE, ".pipeline_lock")
_PIPELINE_LOG = os.path.join(HERE, "pipeline_output.log")   # live log visible in dashboard Logs panel
...
def _ckpt(name): return os.path.join(HERE, "checkpoints", name)
def _data(name): return os.path.join(HERE, "data", name)
...
def _eval_cmd(student_path, label, teacher, datasets="gsm8k",
              ...
              experiment_tag=None, hw_tier="laptop"):
    ...
    hw_tier: passed to run_all.py --hw_tier so results.db rows are tagged correctly.
      laptop  → smoke/correctness runs (Qwen2.5-0.5B -> Qwen3-0.6B, diverse50)
      colab   → trend-formation runs (quantized 4-bit target, T4 15GB)
      a100    → paper-quality runs (bf16 full precision, no quantization)
    """
    cmd = [
...
def build_steps(draft, target, experiment_tag=None, smoke=False, eagle=False,
    ...
    config:     Hardware tier label — determines hw_tier stored in results.db and
                controls whether --load_in_4bit appears in training commands.
                'laptop'       → hw_tier='laptop', no quantization
                'colab'        → hw_tier='colab', --load_in_4bit on train_qwen3.py
                'server'/'a100'→ hw_tier='a100', NO --load_in_4bit (full bf16 only)
    """
    # Determine hw_tier for results.db tagging
    # NOTE: server config maps to a100 tier — same hardware class, different name.
    _hw_tier = {"laptop": "laptop", "colab": "colab",
                "server": "a100", "a100": "a100"}.get(config, "laptop")

    # --load_in_4bit is COLAB-ONLY: loads the 8B target in 4-bit NF4 to fit on T4.
    # NEVER pass this flag for server/a100 config — bf16 full precision is required
    # for paper-quality numbers. The guard in run_all.py --hw_tier a100 will also
    # catch this if quantized results are accidentally tagged as a100.
    _use_4bit = (config == "colab")
    ...
    return [
        ...
        # -------------------------------------------------------------------
        {
            ...
            "cmd": [sys.executable, "train_qwen3.py", "--merge_only",
        ...
        # -------------------------------------------------------------------
        {
            ...
            "cmd": [
                ...
                # --load_in_4bit: COLAB ONLY — loads 8B target in 4-bit NF4 for T4 15GB
                # DO NOT add this flag for server/a100 config (full bf16 required for paper)
                *( ["--load_in_4bit"] if _use_4bit else [] ),
            ],
        ...
        # -------------------------------------------------------------------
        {
            ...
            "cmd": [sys.executable, "train_qwen3.py",
                    "--loss", "reverse_kl",
                    "--steps", "1000", "--lr", "3e-5",
                    "--nan_action", "skip",
                    "--early_stop_patience", "3",
                    ...
                    # --load_in_4bit: COLAB ONLY (see note in train_kl_gsm8k)
                    *( ["--load_in_4bit"] if _use_4bit else [] )],
        ...
        # -------------------------------------------------------------------
        {
            ...
            "cmd": [sys.executable, "online_serve.py",
        ...
        # -------------------------------------------------------------------
        # Phase 5 (laptop-smoke only): Laptop Smoke — exercise ALL code paths
        #
        # Purpose: Qwen2.5-0.5B → Qwen3-0.6B, diverse50 dataset.
        # Runs EVERY verifier (traversal, specinfer, gbv, naive, bv) and
        # EVERY loss (forward_kl, reverse_kl, jsd, ebe, l1) for 200 steps each,
        # at ALL LRs (1e-5, 3e-5, 1e-4).  Total ~100-200 steps per combo.
        # Results are NOT paper-quality — purpose is to confirm no crash/OOM.
        # Verifiers naive and bv serve as lower bounds; nss/spectr/khisti are
        # only exercised here (not in colab or a100 paper runs).
        # -------------------------------------------------------------------
        *([
            {
                "id": "laptop_smoke_all_verifiers",
                "group": "Phase 5 — Laptop Smoke (all code paths)",
                "desc": "Smoke eval: ALL verifiers (traversal,specinfer,gbv,naive,bv) on diverse50",
                "cmd": _eval_cmd(
                    draft, "baseline", target,
                    datasets="diverse50",
                    modes="alpha,traversal,specinfer,gbv,bv,naive",
                    Ks="3,5", temps="0.6,1.0",
                    n=10, max_tokens=50,
                    task_score=False,
                    experiment_tag=(experiment_tag or "laptop_smoke"),
                    hw_tier="laptop",
                ),
                "done_check": None,
                "smoke_skip": False,   # this IS the smoke stage — always run when included
            },
            {
                "id": "laptop_smoke_all_losses",
                "group": "Phase 5 — Laptop Smoke (all code paths)",
                "desc": "Smoke train: ALL losses (forward_kl, reverse_kl, jsd, ebe, l1), 200 steps each",
                "cmd": [
                    sys.executable, "train_qwen3.py",
                    "--loss", "forward_kl,reverse_kl,jsd,ebe,l1",
                    "--steps", "200",
                    "--lr", "1e-5,3e-5,1e-4",
                    "--draft", draft,
                    "--target", target,
                    "--dataset", _data("diverse50.jsonl"),
                    "--output", _ckpt("laptop_smoke_losses"),
                    "--nan_action", "skip",
                    # NO --load_in_4bit here — laptop smoke always uses full bf16
                ],
                "done_check": None,
                "smoke_skip": False,
                "retryable": True,
        ...
        # -------------------------------------------------------------------
        # Phase 7 (optional): EAGLE Benchmark — only included when --eagle
        #
        ...
        *([
            {
                ...
                "done_check": os.path.join(HERE, "data", "eagle_tmp", "00000.pt"),
            },
            {
                "id": "eagle_train",
                "group": "Phase 5 — EAGLE Benchmark",
...
_GBV_DIR = os.path.join(os.path.dirname(HERE), "GBV")
...
def run_smoke_preflight(draft, target):
    ...
    data_candidates = [
        os.path.join(HERE, "data", "gsm8k_30.jsonl"),
        os.path.join(HERE, "data", "gsm8k_10.jsonl"),
        os.path.join(HERE, "data", "gsm8k_50.jsonl"),
        os.path.join(HERE, "data", "gsm8k_5.jsonl"),
    ]
    ...
    tmp_path = os.path.join(HERE, "data", "_smoke_preflight_2.jsonl")
...
def run_step(step, state, dry_run=False):
    ...
    try:
        ...
        if rc == 0:
            ...
            _err_path = os.path.join(HERE, f"pipeline_step_{sid}_error.log")
            try:
...
def main():
    ...
    STEPS = build_steps(draft, target, experiment_tag=args.experiment_tag,
                        smoke=args.smoke, eagle=args.eagle, config=args.config)
