...
async function loadData() {
  ...
  // HW_TIER_FILTER: filter by selected hardware tiers
  // Runs with no hw_tier (legacy rows before the column was added) are treated
  // as 'laptop' and shown whenever 'laptop' is in the filter set.
  if (HW_TIER_FILTER.size < 3) {
    runs = runs.filter(r => {
      const tier = r.hw_tier || 'laptop';
      return HW_TIER_FILTER.has(tier);
    });
  }
