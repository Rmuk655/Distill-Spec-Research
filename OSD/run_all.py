...
def run_be_batch(student_path: str, teacher_path: str, data_path: str,
    ...
    _be_log = os.path.join(_HERE, "be_progress.log")
...
def main():
    ...
    p.add_argument("--hw_tier", default="laptop",
                   choices=["laptop", "colab", "a100"],
                   help="Hardware tier for this run (stored in results.db). "
                        "laptop = smoke/correctness (Qwen2.5-0.5B->Qwen3-0.6B, diverse50). "
                        "colab  = trend formation, 4-bit quantized target (T4 15 GB). "
                        "a100   = paper-quality, bf16 full precision, no quantization. "
                        "IMPORTANT: never use a100 with a 4-bit quantized target — "
                        "run_all.py will error on that combination.")
    ...
    # ── A100-tier guard: never allow quantized models in paper-quality tier ──────
    # If --hw_tier a100 is passed with a target that was loaded with 4-bit
    # quantization, the block efficiency numbers are NOT paper quality (quantization
    # degrades throughput and changes acceptance probabilities slightly).
    # This guard catches the most common mistake: copy-pasting a Colab command that
    # includes --load_in_4bit and accidentally tagging it as a100.
    # Note: run_all.py itself never loads models in 4-bit; train_qwen3.py handles
    # that via --load_in_4bit. The guard checks the BitsAndBytesConfig that would
    # have been used if the user manually loaded the teacher with quantization.
    if args.hw_tier == "a100":
        _is_quantized = False
        try:
            from transformers import AutoConfig as _AC
            import json as _json
            _cfg_path = os.path.join(args.teacher, "quantize_config.json")
            if os.path.exists(_cfg_path):
                with open(_cfg_path) as _qf:
                    _qdata = _json.load(_qf)
                if _qdata.get("bits") in (4, 8):
                    _is_quantized = True
        except Exception:
            pass
        # Also check common env variable set by bitsandbytes when 4-bit is active
        if os.environ.get("BNB_4BIT_QUANT") == "1":
            _is_quantized = True
        if _is_quantized:
            print("\n" + "!" * 72)
            print("  FATAL: --hw_tier a100 was requested but the teacher model appears")
            print("  to be quantized (4-bit or 8-bit).  A100-tier results must use full")
            print("  bf16 precision — quantized results must NOT be tagged as a100 tier.")
            print("  Use --hw_tier colab for quantized (T4) runs.")
            print("!" * 72 + "\n")
            sys.exit(1)
