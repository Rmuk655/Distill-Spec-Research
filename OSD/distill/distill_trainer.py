...
class DistillTrainer(Trainer):
    def __init__(self, teacher_model, *args, **kwargs):
        ...
        self._copy_model = None  # lazy-initialized in _get_copy_model()
    ...
    def offline_training_step(self, model, inputs):
        ...
        if sample_mix_token:
            copy_model = self._get_copy_model(model)
    ...
    def _get_copy_model(self, model):
        """Lazy-init a copy of the student model (same architecture) used for inference-only sampling."""
        if self._copy_model is None:
            self._copy_model = transformers.AutoModelForCausalLM.from_config(model.config)
            self._copy_model.cuda()
        self._copy_model.load_state_dict(self.remove_module_prefix(model.state_dict()))
