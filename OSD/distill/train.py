...
def preprocess(
    ...
    if ("llama" in model.lower()) or ("starcoder" in model.lower()):
        ...
        if do_eval:
            input_ids = tokenizer(prompt_texts, return_tensors="pt").input_ids
            targets = input_ids.clone()
            prompt_ids = input_ids
            prompt_attention_mask = input_ids.ne(tokenizer.pad_token_id)
        else:
            input_ids = tokenizer(
                conversations, return_tensors="pt", padding="max_length",
                max_length=tokenizer.model_max_length, truncation=True,
            ).input_ids
            targets = input_ids.clone()
            for i, prompt_text in enumerate(prompt_texts):
                prompt_len = len(tokenizer(prompt_text).input_ids)
                targets[i, :prompt_len] = IGNORE_TOKEN_ID

            tokenizer.padding_side = "left"
            prompts = tokenizer(
                prompt_texts, return_tensors="pt", padding="max_length",
                max_length=tokenizer.model_max_length, truncation=True,
            )
            tokenizer.padding_side = "right"
            prompt_ids = prompts["input_ids"]
