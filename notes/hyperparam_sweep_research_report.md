...
- **Multi-root, tail-reuse estimator — N {16,32} and random-offset, `prob` lr=1e-5:** N=16 vs N=32 (both fixed-offset): 5.699 vs 5.647, Δ=0.052 — within noise on BE, and N=32 doesn't beat N=16, so there's no rising trend with N to extrapolate from (bigger N isn't obviously better in this data, not yet tested past 32). Offset on vs off at N=16: 5.744 vs 5.699, Δ=0.045 — within noise, no offset effect. All three beat the single-root baseline (5.608) by 0.039–0.136, with N=16+random-offset (5.744, Δ=0.136) sitting right at the edge of the noise floor, not a verified win on BE. This is the one family, though, with a real pass@k signal — plain N=16 clears `gsm8k` k16/k32/k64 above jsd (see [passk_prob_vs_jsd_research_note.md](passk_prob_vs_jsd_research_note.md)), consistently across all three N-family variants tested. `N=8` is training now; add once evaluated.
...
- **Single-root M {4,8,16}, `prob` lr=3e-6, all cold-start from `Qwen/Qwen3-0.6B` (CLOSED):** best_val_block_eff — M4=5.701, M8=5.810, M16=5.619/5.694 (two seeds). No trend with `M`: all values (≈5.62–5.81) fit inside the 0.10–0.15 noise floor. An earlier claim that M16 was genuinely worse did not replicate on a second seed. Pass@k cross-check agrees (see [passk_prob_vs_jsd_research_note.md](passk_prob_vs_jsd_research_note.md)): M4/M8/M16(bare) all clear `gsm8k` k16/k32/k64 above jsd, but the M16 replication doesn't clear at all — the pattern tracks individual checkpoints, not `M` itself.
