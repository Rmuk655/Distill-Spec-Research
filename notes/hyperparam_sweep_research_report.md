...
- **Doc-faithful fresh multiroot (`freshM1`, N=16, M=1) does not reproduce tail-reuse's `gsm8k` win — it reverses.** Tail-reuse N=16 clears the `gsm8k` floor at k16/k32/k64 (Δ +0.031 to +0.034). `freshM1` clears nothing there and goes *negative* at k8–k64 (Δ −0.009 to −0.040), falling below jsd rather than just below tail-reuse. `math_eval` is flat for both (nothing clears there for any config, as usual). `olympiad_eval` favors `freshM1` (clears k16/k64, tail-reuse only k4/k8), but that dataset is where almost nothing ever clears project-wide, so it's thin evidence next to the `gsm8k` reversal. Single seed so far. **Update:** `freshM1_ceanneal3000` (best-BE fresh-family point, 5.779) substantially shrinks the `gsm8k` reversal (Δ +0.007 at k8, −0.009/−0.010 at k32/k64, vs bare `freshM1`'s −0.009/−0.028/−0.040) but still clears nothing — anneal moves it to "roughly tied with jsd," not "matches tail-reuse's win." It's worse than bare `freshM1` on `olympiad_eval`, though (clears nothing there vs `freshM1`'s k16/k64) — the anneal's `gsm8k` gain trades off against a small step back on the dataset where fresh previously had an edge. Neither fresh-family checkpoint reproduces tail-reuse's confirmed win yet.

  **Update: fresh multiroot reproduces the win, but only at N=32.** BE for the two newest fresh points: `freshM2` (N=16, M=2) = 5.720/5.671, `N32_freshM1` (N=32, M=1) = 5.677/5.607 — both inside the same 5.6–5.8 noise band as every other fresh/tail-reuse config, no BE surprise. On pass@k, though, `N32_freshM1` **clears the `gsm8k` floor at k32 (Δ=0.030) and k64 (Δ=0.030)** — matching tail-reuse N32's own pattern almost exactly (tail-reuse N32 also only clears k32/k64: Δ=0.022/0.030, not k16). This is the first fresh-multiroot checkpoint to reproduce a real tail-reuse win rather than just narrow the gap. `freshM2` (N=16) doesn't clear anywhere, but isn't a reversal either — its `gsm8k` gaps shrink smoothly toward ~0.000 by k64, flat parity with jsd. Every N=16 fresh variant tested (bare, +ceanneal, M=2) sits at or below parity on `gsm8k`; N=32 is the exception, and tail-reuse shows the same N=16-vs-N=32 asymmetry (N=32 clears fewer/narrower k's there too) — this looks like the effect is real but concentrated at higher N, not that the fresh estimator itself is broken.
