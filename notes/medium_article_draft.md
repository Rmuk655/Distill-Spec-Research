...
- Built a multi-GPU experiment and evaluation pipeline in PyTorch and HuggingFace, with generation served through vLLM
- Developed and experimentally validated hypotheses for why several distillation objectives underperformed in this setting. Some were adapted from published work, others were novel constructions we designed ourselves
...
The project combined research, systems engineering, and experimental design. That meant reproducing published methods, building scalable infrastructure, and figuring out why objectives succeeded or failed.
...
I built a multi-GPU experiment pipeline supporting parallel training, checkpoint recovery after crashes, automated evaluation across every verifier by tree-width by dataset combination, and cross-checkpoint analysis at scale. It started on a memory-constrained Colab T4 (which needed 4-bit quantization to fit the 8B target), moved to Kaggle's dual-T4 setup, then to A100s for production runs. A separate vLLM-based evaluation pipeline generated tens of thousands of samples for pass@k benchmarking. It relies on a paged KV-cache (a cache of each token's attention keys and values so the model doesn't recompute them at every new step) plus continuous batching, which together keep generation fast enough to be usable at that scale. I added resumable, checkpoint-skipping logic on top so a killed run never lost completed work.

A typical experiment cycle looked like this: implement a new loss variant in PyTorch, validate its gradient behavior against a known baseline on a handful of steps, launch a multi-GPU sweep tracked in Weights & Biases, evaluate every resulting checkpoint across the full verifier grid, then write or extend an analysis script to compare acceptance metrics and pass@k against prior runs.
...
I added two further axes: forgetting (regression on previously-solved problems mid-training, a backward-transfer check) and difficulty-bucketed accuracy (easy, medium, hard, split by baseline performance). Both surfaced runs that looked healthy in aggregate but were regressing on problems they'd already solved, or only helping on the easy bucket.

Which dataset I evaluated on mattered as much as which checkpoint I was looking at. I ran everything across three benchmarks spanning very different difficulty levels, from grade-school-level word problems up through a harder competition-math set and finally an olympiad-level set. Both block efficiency and pass@k, and how much training improved either one, shifted noticeably by dataset: the easiest benchmark was close to saturated for every model, so it barely separated good runs from bad ones, while the harder sets showed much more spread in both the baseline scores and the size of the improvement from training.
