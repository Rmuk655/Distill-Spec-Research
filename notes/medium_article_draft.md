...
- Implemented and evaluated dozens of verifier-aware distillation objectives spanning published and novel training methods
...
<img src="medium_chart_architecture.png" width="650" alt="Experiment platform architecture" />
...
The 32B teacher was the harder memory constraint. At full precision it needs roughly 64GB just for weights, leaving little headroom once the student, its optimizer state, and multi rollout activations are added — it does not fit at all on a 40GB card, and barely fits on an 80GB one. As larger GPUs became available, from a 16GB Colab GPU to Kaggle's 30GB to a 40GB A100 to a final 80GB A100, I progressively removed LoRA, optimizer quantization, and finally teacher quantization, ending with full-precision training on A100s.
...
I tried to keep the loop hardware-efficient to move fast, packing whatever could share a GPU onto one. Evaluation turned out to be the exception, and the reason came from a real bug: running multiple evals on the same GPU caused CPU-GPU transfer contention, so the same checkpoint measured differently across parallel evals, run to run — not just slower, actually unreliable. The fix was one eval per GPU. From there, a few more things kept the loop from wasting hardware: packing multiple concurrent training runs onto one GPU when the model pair was small enough to afford it, early-stopping a sweep on a smoothed validation plateau so a dead run frees its GPU instead of idling to a fixed step count, and caching both training and evaluation progress to disk so a killed sweep only redoes the unfinished work.

One systems lesson surprised me: GPU utilization during eval was low, and the bottleneck was CPU-GPU handoff, not the model's matrix multiplications. Production systems solve that with batching, CUDA graphs, or serving stacks like vLLM; our goal was narrower, whether the algorithm itself could improve throughput, so we reported block efficiency instead of tokens/sec, which would have measured my harness's engineering as much as the algorithm.

I thought FlashAttention vs. SDPA was just a speed choice, until it was not: both compute mathematically identical attention but round differently in bf16, and acceptance here is a hard threshold, so a last decimal difference can flip one token and cascade through the rest of a generation. Two otherwise-identical runs, different only in attention backend, diverged by roughly 0.2 block efficiency — enough to look like a real result above the noise floor when it was just kernel level rounding. Fix: pin one backend everywhere and log which ran.
...
The strongest improvement came from the verification side. [Rahul's own published research](https://arxiv.org/abs/2602.16994) introduced a technique that delays where the draft's speculation tree branches, committing to a single stem before splitting into candidates at the point where draft and target distributions actually diverge, instead of branching at the first token — but only ever evaluated on untrained draft models, never trained, distilled ones. Swept across every trained checkpoint, both model pairs, it improved block efficiency on nearly all of them, ranging from a couple percent for verifiers already close to their ceiling to over 20% for verifiers further behind, and stacking it on the multi rollout distillation above gave the best single result in the project.

Rather than grid-searching the branch-point setting, I tracked each checkpoint's own mean acceptance depth and observed that stronger drafts could delay branching to lower depths.
...
After accounting for run to run noise, estimated from near duplicate configurations, we did not observe significant signals across most settings swept.

## Closing

This project gave me a genuine appreciation for the whole system behind ML research: the math formulation behind an objective, verifying it actually works, and the memory, performance, benchmarking, and debugging work needed to test it at real scale. I came out of it confident that I can take on real systems challenges in ML, not just reason about them on paper.
