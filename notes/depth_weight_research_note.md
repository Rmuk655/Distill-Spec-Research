...
**Status:** **Negative overall** — no aggregate BE gain over flat JSD at any λ or K_train, on either dataset. **One pattern worth a confirm run:** depth_weight beats flat *only* on the matched **train-K = eval-K diagonal** (K_train=1 @ K_eval=1: **+0.054, 6/9** verifiers; K_train=3 @ K_eval=3: **+0.067, 8/9**), neutral-to-harmful off-diagonal — and the gain is carried by **budget/competition verifiers, not `naive`** (the probe's own verifier is the worst cell). Single seed, n=100. Mechanism (§why): a *detached* scalar weight is per-prompt LR scaling — it cannot steer the gradient toward higher acceptance, so it cannot work in principle.

**Draft–Teacher:** Qwen3-0.6B / Qwen3-8B | **Train:** math_hard (K_train=1 and 3, ~40K steps) + gsm8k (λ-sweep, 4K) | **Eval:** math_eval + gsm8k_eval, n=100, K_eval=1–4, 9 verifiers.
...
- **Verdict:** depth_weight as a general-purpose loss is **closed**. Before citing the matched-K diagonal, confirm the two diagonal cells (+0.054, +0.067) with a second seed or n=1000 (include the NSS +0.242 outlier). Supported verifiers for the probe (have `expected_*_depths`): naive, nss, specinfer, spectr, khisti, traversal — **not** bv/gbv.
