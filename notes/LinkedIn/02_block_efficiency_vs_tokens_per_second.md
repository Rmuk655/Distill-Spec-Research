...
Our own profiling shows a piece of this, though the full picture needs care. The 32B teacher has four times the parameters of the 8B teacher. Measured across the same 100 prompts, traversal verifier, K=3: the 8B teacher took about 49 milliseconds per target call, the 32B teacher took about 86. Timing alone does not settle compute bound versus memory bound, since a naive model predicts roughly four times either way, both the math and the bytes to stream scale with parameter count together. The number that actually points somewhere is GPU compute utilization, which barely moved, 31.6 percent for the 8B teacher against 33.5 percent for the 32B one, despite four times the parameters. If the tensor cores were the limit, that much more math packed into a similar wall clock time should have pushed utilization up sharply. It stayed flat, which says the compute engines were not the bottleneck in either run, consistent with time being spent moving weights rather than computing on them. Why the timing increase landed well under four times, rather than the four times a same-shape roofline model predicts under either bottleneck, is a real question this data does not fully answer, likely some mix of the 32B model's parameters being spread differently across depth and width, and larger matrix multiplies simply running closer to peak efficiency than smaller ones.

So at the batch size I ran at, decode speed tracks memory bandwidth and interconnect more than raw compute. Move the same model from an A100 to an H100 to a B200, and NVLink bandwidth between GPUs alone jumps from about 900 GB/s to 1.8 TB/s, on top of a faster HBM generation. Same model, same code, newer hardware, more tokens per second, with zero change to the algorithm. That alone makes tokens per second a bad metric for my question.
