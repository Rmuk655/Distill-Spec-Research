...
5. **`05_eval_contention_bug.md`** — Operating systems: two jobs fit on the GPU, and still could not share it. Capacity vs isolation as a scheduling problem: a memory-based placement policy (`_best_free_cuda_device`, picks by free VRAM) that packed evals onto shared GPUs, a fixed-seed checkpoint scoring differently, CPU-contention as the leading (untraced) hypothesis via `cpu_util_avg_pct`, and the fix — one eval per GPU plus a per-eval CPU thread budget (`--cpu_threads` = physical cores / GPU count). Lands on workload-dependent scheduling (training packs, eval runs exclusive) and reproducibility as a system invariant. Second lesson from the same move-fast instinct: unattended jobs die (SIGHUP on dropped SSH, disk-full crashes), so they must survive (nohup/disown) and resume from saved state (`state.json` argv, resumable pass@k) rather than restart. Grounded: `verifiers/util.py` `_best_free_cuda_device`, `eval.py` `--cpu_threads` + 1-eval-per-GPU protocol, `telemetry.py` `cpu_util_avg_pct`, `resume_crashed_runs.sh`/`migrate_ckpt_root.sh` (commit `9976185`).
...
| Operating systems, scheduling, isolation | 5 | capacity vs isolation, memory-based placement, CPU-thread budgeting, workload-dependent scheduling, reproducibility invariant |
