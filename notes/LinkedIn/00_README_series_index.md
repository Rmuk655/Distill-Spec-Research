...
Every title below now leads with the discipline it actually comes from, per the breadth and depth table further down. This is the table of contents a reader sees before opening anything.

1. Platform overview (the hub)
2. Computer architecture: tokens per second measures the system, block efficiency measures the algorithm. Ends on the partial derivative framing, hold every other variable still, let only the one you care about move.
3. Statistics: freezing every other variable was not enough. The escalating argument, sample size, seeds, datasets, model pairs, loss families, for what it actually takes to call an improvement real.
4. Operating systems: the checkpoint that scored differently every run
5. Scientific computing: the 0.2 gain that was a rounding error
6. Capacity planning: fitting a 32B model on one GPU
7. Systems software: what a KV cache is and why vLLM exists
7a. ML fundamentals: most people test one loss against one verifier, we tested 34 against 9
8. ML fundamentals: the loss that matched my metric and still lost
9. ML fundamentals: two training ideas that failed
10. ML fundamentals and systems: the one change that beat the baseline
11. Statistics: when one metric was not enough

## Post 3: what it covers

`03_noise_floor.md` has been rewritten from the original single noise-floor tip into the full statistics lesson. It picks up directly where post 2 left off:
...
- One seed is not enough either. A combined run (JSD plus a depth weighted curriculum plus a tree loss) had a headline result, a claimed +0.293 win, that turned out to be a K=1 number read against the K=3 baseline cell, not the matching one. Compared correctly, the real delta was +0.03, inside noise.
...
- Even a full sweep across many training configurations at once came back with no result clearing the floor almost anywhere, which is what a real search looks like, not a failure.
- And an honest, unmet limitation to end on: every escalation above still only ever touched one family of models, Qwen. A paper level claim needs the same effect checked against other families entirely, Gemma, GPT, Llama, not just more seeds and datasets inside the one family already tested. This is the one escalation the project never got to, stated plainly rather than glossed over.

The honest through line: the first four escalations trace back to the same root cause, the randomness inherent in autoregressive sampling and in the accept or reject draw itself, which survives no matter how carefully everything else is held fixed. The model family point is a different kind of gap, about how far a result generalizes, not about noise, and it is what keeps this a well tested finding on one family rather than a settled one.

Grounding sources for the rewrite: project report noise floor SE figures; `tree_losses_research_note.md` and `interview_prep.md` for the +0.293 to +0.03 correction (the figure and the source were both wrong in an earlier draft of this plan, verified and fixed against these two files, not `depth_weight_research_note.md` which does not contain this specific number); `enrich_research_note.md` for the two seed n=1000 study (s123 vs s456) and the 0.6/8 vs 1.7/32 replication failure; the explicit n=1 caveat on every 1.7B/32B number; `passk_prob_vs_jsd_research_note.md` for the "no significant signal across most settings" finding, which is the same lesson applied to an entire study, not just one metric.
