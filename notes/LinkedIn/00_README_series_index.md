...
7. **`07_systems_at_scale.md`** — Capacity planning: fitting a 32B model, then serving it. Memory budget across four GPU tiers; the three memory-reduction techniques and their catches (LoRA; 4-bit NF4 teacher, which cost ~18x on flat losses since generate re-dequantizes every step, 18.3s vs 1.0s/step; 8-bit optimizer state, plus the reverted startup-warmup OOM); KV cache, vLLM, and the honest orchestration-versus-distributed-training distinction (every model fit on one GPU, no NCCL, no tensor/pipeline parallelism). Grounded: the 16GB Colab to 80GB A100 progression; commits `d822c25` (--optim_8bit, 18x figure), `11f2d84`/`633032e` (4-bit teacher), `f536924` (reverted warmup); `passk` pipeline commits (async vLLM, isolated venv, resumable).
