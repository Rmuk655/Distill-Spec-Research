...
5. **`05_eval_contention_bug.md`** — Operating systems: getting 500 experiments through a handful of GPUs. Chronological infra progression, not a single bug story: manual babysitting as the "before" state, grounded in a real 40,000-step (10x default) depth-weight ablation baseline run the old way (`results/jsd_jsd_dw_naive_tree_results_math_hard_40000_steps.csv`, commit `6c1270a`); `nohup` plus a `--resume` hardened through real bugs (silent try_resume/wandb failures; crash recovery needing only the last checkpoint since `state.json` already stores the launch command and W&B run name, commit `9976185`); a memory-aware scheduler (`_best_free_cuda_device`, picks by free VRAM) for jobs small enough to share a card, versus manual `CUDA_VISIBLE_DEVICES` pinning for the 1.7B/32B pair (needs a full 80GB card) and for hyperparameter/eval sweeps launched as parallel `nohup` jobs one per GPU index; evaluation as the one place packing did not belong, sourced to Rahul's real warning that sharing a GPU makes a throughput measurement meaningless (not an unverified block-efficiency claim, which earlier drafts dropped since no CSV ever backed it), traced to CPU thread oversubscription and fixed with one eval per GPU plus a per-eval CPU thread budget (`--cpu_threads` = physical cores / GPU count); and early stopping as the fail-fast finish (`--early_stop_patience` first added `5a78043`, raised 5 to 15 checks `0649ece`, warmup guard + `--early_stop_min_delta` `20aaa76`, `--divergence_abort_frac`/`--min_steps` floor `3cef326`). Lands on aggregate experiment throughput versus per-job speed, and utilization versus useful, reproducible throughput as the real distinction, tying back to Post 3's algorithmic-efficiency-versus-system-throughput lesson. Grounded: `verifiers/util.py` `_best_free_cuda_device`, `eval.py` `--cpu_threads` + 1-eval-per-GPU protocol, `train.py` resume/early-stop flags, commits `7591a4c`/`46a253f`/`35192dc`/`ecbfae4`/`5a78043`/`20aaa76`/`0649ece`/`3cef326`/`9976185`.
