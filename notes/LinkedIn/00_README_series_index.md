...
7. **`07_systems_at_scale.md`** — Capacity planning: fitting a 32B model, then serving it. Two honest phases: (1) free-tier scaffolding (laptop/Colab/Kaggle T4 with 1.7B/4B BF16 teachers, no full training there) and the real constraint that 4-bit NF4 OOMs Colab's ~12GB CPU RAM, so smaller BF16 teachers were used instead; (2) the A100 era where real training happened and the three memory tools earned their keep scaling the teacher 8B→32B (LoRA; 4-bit NF4 teacher, ~18x flat-loss slowdown since generate re-dequantizes every step, 18.3s vs 1.0s/step; 8-bit optimizer state + the reverted startup-warmup OOM). Then KV cache, vLLM, and the orchestration-versus-distributed-training distinction (every model fit on one GPU, no NCCL, no tensor/pipeline parallelism). Grounded: `deploy/PROGRESSION.md` + tier YAML configs (all `load_in_4bit: false`, teacher-size scaling), empty `deploy/RUN_LOG.md` (scaffolding, not runs); commits `d822c25` (--optim_8bit, 18x), `11f2d84`/`633032e` (4-bit teacher), `f536924` (reverted warmup); `passk` pipeline commits.
...
| Systems at scale, memory, serving | 7 | free-tier scaffolding vs A100 training, memory tools for the 8B→32B teacher, KV cache, vLLM, orchestration |
