...
The 80GB A100 was where it came together: the bf16 teacher fit there without quantizing, and I could train the whole draft, a full fine tune, not adapters. But a full precision AdamW optimizer on top of a 64GB teacher still ran over 80GB, since Adam keeps two fp32 statistics per parameter. So I used a [bitsandbytes 8 bit optimizer](https://huggingface.co/docs/bitsandbytes/explanations/optimizers), storing them in 8 bit to bring the peak under the card. My first version allocated that state at startup, before the loading spike had cleared, and hit a GPU out of memory error on the first step; letting it allocate lazily fixed that. A bf16 teacher, a full fine tune draft, and an 8 bit optimizer held the 32B pair on one card.
