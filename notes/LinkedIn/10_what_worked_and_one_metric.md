...
That pipeline had its own systems problem. The obvious version, load the model and generate in a loop, was far too slow for the tens of thousands of samples pass at k needs. I used [vLLM](https://arxiv.org/abs/2309.06180) instead, which keeps the GPU busy two ways: paged attention stores each sequence's KV cache in small reusable blocks so memory does not fragment, and continuous batching admits a new request the moment an old one finishes instead of waiting for a whole batch to end. Its dependencies conflicted with my training stack, so it ran in a separate environment with its own resume logic, so a killed job never regenerated work it had already finished.

![The platform: inference, distillation training, and the evaluation pipeline built around it](../medium_chart_architecture.png)
*The evaluation pipeline is the bottom row: each trained checkpoint fans out to the block efficiency eval and the pass at k eval, the second running on vLLM with a batched KV cache in its own isolated environment. The top rows show where it sits, the speculative decoding inference loop and the distillation training that produces the checkpoints.*
