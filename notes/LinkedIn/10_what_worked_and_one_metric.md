...
Block efficiency, my main metric, measures one thing: how well the draft matches the target's output. It does not measure whether the draft actually got better at solving problems. Those sound like the same thing. They are not, and this should not be shocking once you say it that plainly, block efficiency is a single step distribution match and solving a problem is a whole answer correctness check, there was never a guarantee they would move together. What matters is that it actually happened, with real checkpoints, not just as a hypothetical risk.
...
This revealed things block efficiency could not see. Some checkpoints got better at getting the answer right on a single try, but worse when given many tries. Training had sharpened them onto one confident answer and narrowed their variety, which is exactly the wrong thing if you are going to sample many times. My main metric was happy. Pass at k showed the hidden cost. This was not universal. Most trained checkpoints beat the untrained student across the board, but at least two specific checkpoints on the harder math set were beaten by the untrained student once k climbed past about sixteen, the untrained student's curve was simply steeper. Other checkpoints, also trained, did not show this at all and stayed ahead the whole way, so it is a property of specific checkpoints, not of training itself.
...
Here it is, named plainly instead of left implicit. LLM inference is a coupled system: training objective, draft distribution, speculation tree, verifier, kernels and numerics, hardware, and the evaluation metric itself are all links in one chain, and improving one link does not guarantee the chain improves end to end. The rounding error post was the numerics link quietly lying to the metric. This post is two separate examples at once, the verifier link mattering more than the training link everyone assumes matters most, and the evaluation link splitting into two disagreeing measurements. None of these are surprising in isolation. What is worth taking seriously is how often real research time goes into exactly this, checking whether a chain that looks simple on a whiteboard actually holds together once every link is measured separately.
