...
One more layer here, and it is worth keeping as two separate stories rather than folding it into the one above. For flat losses, both backends run fine, they just round differently, which is the whole 0.2 swing already described. For tree losses it was not a rounding choice at all: the draft's custom tree mask forward pass needs a dict based attention mask, which FlashAttention does not support, so SDPA had to be forced specifically on that code path, not picked for speed or numerics. One story is about two correct implementations disagreeing in the last decimal. The other is about one implementation not being usable there at all. Mixing them up would have been its own quiet way to compare the wrong things.
