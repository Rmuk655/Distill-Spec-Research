...
- **⚠ Attn-backend (verified 2026-07):** every enrich file is **sdpa**, but the per-checkpoint 0.6/8 JSD baseline (`jsd_mathhard_s123.csv`) is **FA2 for all math_eval base-verifier cells** (sdpa only for olympiad K2–4). The measured sdpa−FA2 gap ranges −0.37…+0.23 per cell (mean ≈ −0.02), so **individual math_eval Δ cells carry up to ±0.37 backend uncertainty, but multi-cell means are robust** (the gap nets ~0). The **OOD win is matched-backend-verified**: olympiad traversal K2/K3/K4 are sdpa-to-sdpa (+0.054/**+0.286**/+0.009), so the +0.116 OOD mean is not a backend artifact. The **1.7/32 `enrich_K3` deltas are cross-backend at K1–3** (enrich sdpa vs baseline FA2), which only reinforces "does not cleanly replicate" — its lone +0.27 (K3) could be inflated by backend. To fully remove the math_eval caveat, re-run the 0.6/8 JSD baseline base-verifiers under sdpa.
