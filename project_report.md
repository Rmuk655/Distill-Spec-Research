...
| Depth-weighting | `--aux_mode depth_weight --aux_loss <tree_loss> [--depth_linear \| --depth_lambda λ]` | Does curriculum-reweighting JSD by expected acceptance depth help? | No detectable signal alone (detached scalar, can't redirect gradient direction). One combined run (`jsd + depth_weight_lin + naive_tree`, K3, 40K steps) shows **no** verified traversal win at matched K — the +0.29 previously cited was a K_eval=1 BE (6.163) mislabeled as K=3; the true K_eval=3 Δ is +0.03, within noise. | [`depth_weight_research_note.md`](notes/depth_weight_research_note.md) |
