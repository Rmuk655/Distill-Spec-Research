...
analyze_passk_movement.py — two questions the charts alone don't answer directly:
  ...
  2. Did BE and pass@k8/k16 move in a correlated fashion across the sweep, or
     ...
     group: BE range vs pass@k8/k16 range, so you can see e.g. "LR moved both,
     warmup moved neither."
...
summarize_sweep.py's table -- pass --best_be_csv to override with a fresher export).
...
def main():
    ...
    L.append("range = max−min across the group's checkpoints. Large BE range with tiny "
              "pass@k range (or vice versa) = NOT correlated for that knob.\n")
    ...
    for gname, frags in GROUPS.items():
        ...
        k8s = [passk_at(rows, f, args.dataset, 8) for f in frags]
        k16s = [passk_at(rows, f, args.dataset, 16) for f in frags]
        k8s = [v for v in k8s if v is not None]
        k16s = [v for v in k16s if v is not None]
        ...
        if be_range is not None and k8_range is not None:
            # crude call: both "big" or both "small" relative to typical noise scales
            be_big = be_range > 0.05
            pk_big = (k8_range or 0) > 0.03 or (k16_range or 0) > 0.03
