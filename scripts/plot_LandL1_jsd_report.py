#!/usr/bin/env python
"""
plot_LandL1_jsd_report.py -- jsd-only L/L1 sweep, rerun data
(results/passK/LandL1sweepJsd.csv = l_sweep_jsd_rerun.csv [L1=0, all 9
verifiers] + l1_sweep_jsd_rerun.csv [L1=6/8/9/10/12, traversal+specinfer],
checkpoint jsd_lr1e-5_wu10_lrmin0.1_wd0.01, L=16/32).
...
USAGE:
    python scripts/plot_LandL1_jsd_report.py
"""
import csv
import os
...
def main():
    rows = load("results/passK/LandL1sweepJsd.csv")
    ...
    for dataset in DATASETS:
        ...
        ax1.set_title(f"jsd checkpoint: BE and throughput vs L1, at L=16/32 -- {dataset}", fontsize=10)
        ax1.grid(alpha=0.3)
        ax1.legend(handles=handles, fontsize=7, loc="center left", bbox_to_anchor=(1.08, 0.5))
        fig.tight_layout()
        fig.savefig(os.path.join(outdir, f"LandL1_jsd_report_{dataset}.png"), dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"wrote LandL1_jsd_report_{dataset}.png")
